...
public class ProfileController {
	...
	@PutMapping("/{id}/{phone}")
	public ResponseEntity<Object> putProfilePhone(@PathVariable("id") int id,
			@PathVariable("phone") long phone) {
		try {
			Profile profile = profileService.changeProfilePhone(id, phone);
			return ResponseEntity.status(HttpStatus.OK).body(profile);
		} catch (ProfileNotFoundException e) {
			// Exception has getMessage() that returns the message initialized when exception occurs
			Map<String, String> map = new HashMap<String, String>();
	...
	public ResponseEntity<Object> deleteFriend(@PathVariable("profileId") int profileId, 
			@PathVariable("friendId") int friendId) {
		try {
			Friend deletedFriend = friendService.removeFriend(profileId, friendId);
